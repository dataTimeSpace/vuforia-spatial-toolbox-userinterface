...
export const MAX_MURI_SCORE =
    MAX_MURI_SCORE_TRUNK +
    MAX_MURI_SCORE_HEAD +
    MAX_MURI_SCORE_LEGS +
...
export class MuriScore {
    ...
    scoreTrunk() {
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.TRUNK_SIDE_BEND)) {
            if (
                Math.abs(this.data.angles[ERGO_ANGLES.TRUNK_SIDE_BEND]) <
                MURI_CONFIG.trunkSideBendAngleThresholds[0]
            ) {
                this.muriScores[MURI_SCORES.TRUNK_SIDE_BEND] = MURI_CONFIG.scoreWeights[0]; // low strain
            } else if (
                Math.abs(this.data.angles[ERGO_ANGLES.TRUNK_SIDE_BEND]) <
                MURI_CONFIG.trunkSideBendAngleThresholds[1]
            ) {
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.TRUNK_TWIST)) {
            if (
                Math.abs(this.data.angles[ERGO_ANGLES.TRUNK_TWIST]) <
                MURI_CONFIG.trunkTwistAngleThresholds[0]
            ) {
                this.muriScores[MURI_SCORES.TRUNK_TWIST] = MURI_CONFIG.scoreWeights[0]; // low strain
            } else if (
                Math.abs(this.data.angles[ERGO_ANGLES.TRUNK_TWIST]) <
                MURI_CONFIG.trunkTwistAngleThresholds[1]
            ) {
        ...
        trunkScore = Math.max(
            this.muriScores[MURI_SCORES.TRUNK_FRONT_BEND],
        ...
        if (
            this.muriScores[MURI_SCORES.TRUNK_FRONT_BEND] != null ||
            this.muriScores[MURI_SCORES.TRUNK_SIDE_BEND] != null ||
            this.muriScores[MURI_SCORES.TRUNK_TWIST] != null
        ) {
    ...
    scoreHead() {
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.HEAD_SIDE_BEND)) {
            if (
                Math.abs(this.data.angles[ERGO_ANGLES.HEAD_SIDE_BEND]) <
                MURI_CONFIG.headSideBendAngleThresholds[0]
            ) {
                this.muriScores[MURI_SCORES.HEAD_SIDE_BEND] = MURI_CONFIG.scoreWeights[0]; // low strain
            } else if (
                Math.abs(this.data.angles[ERGO_ANGLES.HEAD_SIDE_BEND]) <
                MURI_CONFIG.headSideBendAngleThresholds[1]
            ) {
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.HEAD_TWIST)) {
            if (
                Math.abs(this.data.angles[ERGO_ANGLES.HEAD_TWIST]) <
                MURI_CONFIG.headTwistAngleThresholds[0]
            ) {
                this.muriScores[MURI_SCORES.HEAD_TWIST] = MURI_CONFIG.scoreWeights[0]; // low strain
            } else if (
                Math.abs(this.data.angles[ERGO_ANGLES.HEAD_TWIST]) <
                MURI_CONFIG.headTwistAngleThresholds[1]
            ) {
        ...
        // note: score == null is mapped to 0
        headScore = Math.max(
            this.muriScores[MURI_SCORES.HEAD_FRONT_BEND],
        ...
        // select color for the score of the highest strain (if any of strains is calculated)
        if (
            this.muriScores[MURI_SCORES.HEAD_FRONT_BEND] != null ||
            this.muriScores[MURI_SCORES.HEAD_SIDE_BEND] != null ||
            this.muriScores[MURI_SCORES.HEAD_TWIST] != null
        ) {
    ...
    scoreUpperArms() {
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.LEFT_UPPER_ARM_FRONT_RAISE)) {
            if (
                Math.abs(this.data.angles[ERGO_ANGLES.LEFT_UPPER_ARM_FRONT_RAISE]) <
                MURI_CONFIG.upperArmFrontRaiseAngleThresholds[0]
            ) {
                this.muriScores[MURI_SCORES.LEFT_UPPER_ARM_FRONT_RAISE] =
                    MURI_CONFIG.scoreWeights[0]; // low strain
            } else if (
                Math.abs(this.data.angles[ERGO_ANGLES.LEFT_UPPER_ARM_FRONT_RAISE]) <
                MURI_CONFIG.upperArmFrontRaiseAngleThresholds[1]
            ) {
                this.muriScores[MURI_SCORES.LEFT_UPPER_ARM_FRONT_RAISE] =
                    MURI_CONFIG.scoreWeights[1]; // medium strain
            } else {
                this.muriScores[MURI_SCORES.LEFT_UPPER_ARM_FRONT_RAISE] =
                    MURI_CONFIG.scoreWeights[2]; // high strain
            }
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.LEFT_UPPER_ARM_SIDE_RAISE)) {
            if (
                Math.abs(this.data.angles[ERGO_ANGLES.LEFT_UPPER_ARM_SIDE_RAISE]) <
                MURI_CONFIG.upperArmSideRaiseAngleThresholds[0]
            ) {
                this.muriScores[MURI_SCORES.LEFT_UPPER_ARM_SIDE_RAISE] =
                    MURI_CONFIG.scoreWeights[0]; // low strain
            } else if (
                Math.abs(this.data.angles[ERGO_ANGLES.LEFT_UPPER_ARM_SIDE_RAISE]) <
                MURI_CONFIG.upperArmSideRaiseAngleThresholds[1]
            ) {
                this.muriScores[MURI_SCORES.LEFT_UPPER_ARM_SIDE_RAISE] =
                    MURI_CONFIG.scoreWeights[1]; // medium strain
            } else {
        ...
        // select color for the score of the highest strain (if any of strains is calculated)
        if (
            this.muriScores[MURI_SCORES.LEFT_UPPER_ARM_FRONT_RAISE] != null ||
            this.muriScores[MURI_SCORES.LEFT_UPPER_ARM_SIDE_RAISE] != null
        ) {
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.RIGHT_UPPER_ARM_FRONT_RAISE)) {
            if (
                Math.abs(this.data.angles[ERGO_ANGLES.RIGHT_UPPER_ARM_FRONT_RAISE]) <
                MURI_CONFIG.upperArmFrontRaiseAngleThresholds[0]
            ) {
                this.muriScores[MURI_SCORES.RIGHT_UPPER_ARM_FRONT_RAISE] =
                    MURI_CONFIG.scoreWeights[0]; // low strain
            } else if (
                Math.abs(this.data.angles[ERGO_ANGLES.RIGHT_UPPER_ARM_FRONT_RAISE]) <
                MURI_CONFIG.upperArmFrontRaiseAngleThresholds[1]
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.RIGHT_UPPER_ARM_SIDE_RAISE)) {
            if (
                Math.abs(this.data.angles[ERGO_ANGLES.RIGHT_UPPER_ARM_SIDE_RAISE]) <
                MURI_CONFIG.upperArmSideRaiseAngleThresholds[0]
            ) {
                this.muriScores[MURI_SCORES.RIGHT_UPPER_ARM_SIDE_RAISE] =
                    MURI_CONFIG.scoreWeights[0]; // low strain
            } else if (
                Math.abs(this.data.angles[ERGO_ANGLES.RIGHT_UPPER_ARM_SIDE_RAISE]) <
                MURI_CONFIG.upperArmSideRaiseAngleThresholds[1]
        ...
        // select color for the score of the highest strain (if any of strains is calculated)
        if (
            this.muriScores[MURI_SCORES.RIGHT_UPPER_ARM_FRONT_RAISE] != null ||
            this.muriScores[MURI_SCORES.RIGHT_UPPER_ARM_SIDE_RAISE] != null
        ) {
        ...
        this.data.boneScores[getBoneName(JOINT_CONNECTIONS.shoulderElbowRight)] =
            rightUpperArmScore;
        this.data.boneColors[getBoneName(JOINT_CONNECTIONS.shoulderElbowRight)] =
            rightUpperArmColor;
    }
    ...
    scoreLowerArms() {
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.LEFT_LOWER_ARM_BEND)) {
            if (
                this.data.angles[ERGO_ANGLES.LEFT_LOWER_ARM_BEND] <
                MURI_CONFIG.lowerArmBendAngleThresholds[0]
            ) {
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.LEFT_LOWER_ARM_TWIST)) {
            if (
                this.data.angles[ERGO_ANGLES.LEFT_LOWER_ARM_TWIST] <
                MURI_CONFIG.lowerArmTwistAngleThresholds[0]
            ) {
        ...
        // for now it is the maximum from different types of strain to have a high sensitivity
        // note: score == null is mapped to 0
        leftLowerArmScore = Math.max(
            this.muriScores[MURI_SCORES.LEFT_LOWER_ARM_BEND],
            this.muriScores[MURI_SCORES.LEFT_LOWER_ARM_TWIST]
        );
        ...
        // select color for the score of the highest strain (if any of strains is calculated)
        if (
            this.muriScores[MURI_SCORES.LEFT_LOWER_ARM_BEND] != null ||
            this.muriScores[MURI_SCORES.LEFT_LOWER_ARM_TWIST] != null
        ) {
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.RIGHT_LOWER_ARM_BEND)) {
            if (
                this.data.angles[ERGO_ANGLES.RIGHT_LOWER_ARM_BEND] <
                MURI_CONFIG.lowerArmBendAngleThresholds[0]
            ) {
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.RIGHT_LOWER_ARM_TWIST)) {
            if (
                this.data.angles[ERGO_ANGLES.RIGHT_LOWER_ARM_TWIST] <
                MURI_CONFIG.lowerArmTwistAngleThresholds[0]
            ) {
        ...
        // for now it is the maximum from different types of strain to have a high sensitivity
        // note: score == null is mapped to 0
        rightLowerArmScore = Math.max(
            this.muriScores[MURI_SCORES.RIGHT_LOWER_ARM_BEND],
            this.muriScores[MURI_SCORES.RIGHT_LOWER_ARM_TWIST]
        );
        ...
        // select color for the score of the highest strain (if any of strains is calculated)
        if (
            this.muriScores[MURI_SCORES.RIGHT_LOWER_ARM_BEND] != null ||
            this.muriScores[MURI_SCORES.RIGHT_LOWER_ARM_TWIST] != null
        ) {
    ...
    scoreHands() {
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.LEFT_HAND_FRONT_BEND)) {
            if (
                this.data.angles[ERGO_ANGLES.LEFT_HAND_FRONT_BEND] <
                MURI_CONFIG.handFrontBendAngleThresholds[0]
            ) {
                this.muriScores[MURI_SCORES.LEFT_HAND_FRONT_BEND] = MURI_CONFIG.scoreWeights[2]; // high strain
            } else if (
                this.data.angles[ERGO_ANGLES.LEFT_HAND_FRONT_BEND] <
                MURI_CONFIG.handFrontBendAngleThresholds[1]
            ) {
        ...
        // for now it is the maximum from different types of strain to have a high sensitivity
        // note: score == null is mapped to 0
        leftHandScore = Math.max(
            this.muriScores[MURI_SCORES.LEFT_HAND_FRONT_BEND],
            this.muriScores[MURI_SCORES.LEFT_HAND_REACH]
        );
        ...
        // select color for the score of the highest strain (if any of strains is calculated)
        if (
            this.muriScores[MURI_SCORES.LEFT_HAND_FRONT_BEND] != null ||
            this.muriScores[MURI_SCORES.LEFT_HAND_REACH] != null
        ) {
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.RIGHT_HAND_FRONT_BEND)) {
            if (
                this.data.angles[ERGO_ANGLES.RIGHT_HAND_FRONT_BEND] <
                MURI_CONFIG.handFrontBendAngleThresholds[0]
            ) {
        ...
        // for now it is the maximum from different types of strain to have a high sensitivity
        // note: score == null is mapped to 0
        rightHandScore = Math.max(
            this.muriScores[MURI_SCORES.RIGHT_HAND_FRONT_BEND],
            this.muriScores[MURI_SCORES.RIGHT_HAND_REACH]
        );
        ...
        // select color for the score of the highest strain (if any of strains is calculated)
        if (
            this.muriScores[MURI_SCORES.RIGHT_HAND_FRONT_BEND] != null ||
            this.muriScores[MURI_SCORES.RIGHT_HAND_REACH] != null
        ) {
    ...
    scoreLegs() {
        ...
        if (this.data.offsets.hasOwnProperty(ERGO_OFFSETS.LEFT_TO_RIGHT_FOOT)) {
            const footHeightDifference = Math.abs(
                this.data.offsets[ERGO_OFFSETS.LEFT_TO_RIGHT_FOOT].y
            );
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.LEFT_LOWER_LEG_BEND)) {
            if (
                this.data.angles[ERGO_ANGLES.LEFT_LOWER_LEG_BEND] <
                MURI_CONFIG.lowerLegBendAngleThresholds[0]
            ) {
        ...
        if (this.data.angles.hasOwnProperty(ERGO_ANGLES.RIGHT_LOWER_LEG_BEND)) {
            if (
                this.data.angles[ERGO_ANGLES.RIGHT_LOWER_LEG_BEND] <
                MURI_CONFIG.lowerLegBendAngleThresholds[0]
            ) {
        ...
        if (
            this.muriScores[MURI_SCORES.FEET_HEIGHT_DIFFERENCE] != null ||
            this.muriScores[MURI_SCORES.LEFT_LOWER_LEG_BEND] != null ||
            this.muriScores[MURI_SCORES.RIGHT_LOWER_LEG_BEND] != null
        ) {
            // note: score == null is mapped to 0
            this.muriScores[MURI_SCORES.LEGS] = Math.max(
                this.muriScores[MURI_SCORES.FEET_HEIGHT_DIFFERENCE],
                this.muriScores[MURI_SCORES.LEFT_LOWER_LEG_BEND],
                this.muriScores[MURI_SCORES.RIGHT_LOWER_LEG_BEND]
            );
        }
    ...
    getOverallColor(overallScore) {
        ...
        const fraction =
            (clamp(overallScore, lowCutoff, highCutoff) - lowCutoff) / (highCutoff - lowCutoff);
