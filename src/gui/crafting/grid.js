...
(function (exports) {
    ...
    Cell.prototype.getColorHSL = function () {
        ...
        var colorMap = {
            blue: { h: 180, s: 100, l: 60 },
            green: { h: 122, s: 100, l: 60 },
            yellow: { h: 59, s: 100, l: 60 },
            red: { h: 333, s: 100, l: 60 },
        };
    ...
    Cell.prototype.countHorizontalRoutes = function () {
        return this.routeSegments.filter(function (value) {
            return value.containsHorizontal;
        }).length;
    };
    ...
    Cell.prototype.countVerticalRoutes = function (excludeStartPoints, excludeEndPoints) {
        return this.routeSegments.filter(function (value) {
            return (
                value.containsVertical &&
                !((value.isStart && excludeStartPoints) || (value.isEnd && excludeEndPoints))
            );
        }).length;
    };
    ...
    Cell.prototype.blockAtThisLocation = function () {
        if (this.isMarginCell()) {
            ...
            if (
                blockBefore &&
                blockAfter &&
                realityEditor.gui.crafting.eventHelper.areBlocksEqual(blockBefore, blockAfter)
            ) {
                return blockBefore;
            }
    ...
    Route.prototype.addLocation = function (col, row) {
        ...
        this.cellLocations.forEach(function (cellLocation) {
            if (cellLocation.col === col && cellLocation.row === row) {
                // implicitly prevent duplicate points from being added
                skip = true;
            }
        });
        if (!skip) {
    ...
    Route.prototype.getXYPositionAtPercentage = function (percent) {
        ...
        if (percent >= 0 && percent <= 1) {
            ...
            var alpha =
                percentOver /
                (pointData.percentages[indexBefore + 1] - pointData.percentages[indexBefore]);
    ...
    Grid.prototype.getPixelDimensions = function () {
        var width =
            Math.ceil(this.size / 2) * this.blockColWidth +
            Math.floor(this.size / 2) * this.marginColWidth;
        var height =
            Math.ceil(this.size / 2) * this.blockRowHeight +
            Math.floor(this.size / 2) * this.marginRowHeight;
        return {
            width: width,
            height: height,
        };
    ...
    Grid.prototype.getCellCenterX = function (cell) {
        var leftEdgeX = 0;
        if (cell.location.col % 2 === 0) {
            // this is a block cell
            leftEdgeX = (cell.location.col / 2) * (this.blockColWidth + this.marginColWidth);
            return this.xMargin + leftEdgeX + this.blockColWidth / 2;
        } else {
            // this is a margin cell
            leftEdgeX =
                Math.ceil(cell.location.col / 2) * this.blockColWidth +
                Math.floor(cell.location.col / 2) * this.marginColWidth;
    ...
    Grid.prototype.getCellCenterY = function (cell) {
        ...
        if (cell.location.row % 2 === 0) {
            ...
            // this is a margin cell
            topEdgeY =
                Math.ceil(cell.location.row / 2) * this.blockRowHeight +
                Math.floor(cell.location.row / 2) * this.marginRowHeight;
    ...
    Grid.prototype.getCellsBetween = function (cell1, cell2) {
        ...
        if (this.areCellsHorizontal(cell1, cell2)) {
            ...
            cellsBetween.push.apply(
                cellsBetween,
                this.cells.filter(function (cell) {
                    return (
                        cell.location.row === cell1.location.row &&
                        cell.location.col > minCol &&
                        cell.location.col < maxCol
                    );
                })
            );
            ...
            cellsBetween.push.apply(
                ...
                this.cells.filter(function (cell) {
                    return (
                        cell.location.col === cell1.location.col &&
                        cell.location.row > minRow &&
                        cell.location.row < maxRow
                    );
                })
            );
        }
    ...
    Grid.prototype.getLineSegmentDirections = function (prevCell, currentCell, nextCell) {
        ...
        if (
            this.areCellsHorizontal(currentCell, prevCell) ||
        ...
        if (
            this.areCellsVertical(currentCell, prevCell) ||
    ...
    Grid.prototype.getCellsOver = function (
        firstCell,
        blockWidth,
        itemSelected,
        includeMarginCells
        ...
        for (
            var col = firstCell.location.col;
            col < firstCell.location.col + 2 * blockWidth - 1;
            col += increment
        ) {
    ...
    Grid.prototype.calculateLinkRoute = function (link) {
        ...
        if (startLocation.row < endLocation.row) {
            ...
            var areBlocksBetweenInStartColumn = this.areBlocksBetween(
                this.getCell(startLocation.col, startLocation.row),
                this.getCell(startLocation.col, endLocation.row)
            );
            ...
            if (startLocation.col !== endLocation.col || areBlocksBetweenInStartColumn) {
                ...
                if (firstBlockBelow) {
                    var firstBlockRowBelow = convertBlockPosToGridPos(
                        firstBlockBelow.x,
                        firstBlockBelow.y
                    ).row;
            ...
            if (startLocation.row < this.size - 1) {
                // first point is vertically below the start, except for bottom row
            ...
            if (endLocation.row > 0) {
                ...
                // if it's going to the top row, approach from the side rather than above it

                // if there's nothing blocking the line from getting to the side of the end block, last point goes there
                var cellsBetween = this.getCellsBetween(
                    this.getCell(startLocation.col, 0),
                    this.getCell(endLocation.col, endLocation.row)
                );
                ...
                if (blocksBetween.length === 0) {
                    ...
                    // final exception! if there are blocks horizontally between start and end in top row, go under and up
    ...
    Grid.prototype.calculateAllCellsContainingRoute = function (route) {
        ...
        for (var i = 0; i < route.cellLocations.length; i++) {
            ...
            if (i > 0) {
                prevCell = this.getCell(
                    route.cellLocations[i - 1].col,
                    route.cellLocations[i - 1].row
                );
            }
            ...
            ); // corners have both vertical and horizontal. end point has only vertical //todo: except for top/bottom row?
            if (prevCell === null) {
        ...
        for (var c = 0; c < this.size; c++) {
            ...
            horizontallySortedLinks = this.allLinks().sort(function (link1, link2) {
                ...
                if (p1.vertical >= 0 && p2.vertical >= 0) {
                    if (dCol1 === 0 && dCol2 === 0) {
                        // in start col, bottom -> last
                        return verticalOrder;
                    }
                    if (dCol1 === 0 && dCol2 !== 0) {
                        // lines to right of start col -> last, those to left -> first
                        return -1 * dCol2;
                    }
                    var diagonalOrder;
                    if (dCol1 > 0 && dCol2 > 0) {
                        // to right of start col, topright diagonal bands -> last
                        diagonalOrder = horizontalOrder - verticalOrder;
                        if (diagonalOrder === 0) {
                            // within same diagonal band, top -> last
                    ...
                    if (dCol1 < 0 && dCol2 < 0) {
                        // to left of start col, bottomright diagonal bands -> last
                        diagonalOrder = horizontalOrder + verticalOrder;
                        if (diagonalOrder === 0) {
                            // within same diagonal band, bottom -> last
                            return verticalOrder;
                        } else {
                            return diagonalOrder;
                        }
            ...
            horizontallySortedLinks.forEach(function (link) {
                ...
                var routeCellsInThisCol = link.route.allCells.filter(function (cell) {
                    return cell.location.col === c;
                });
                if (routeCellsInThisCol.length > 0) {
                    // does this route contain this column?
                    ...
                    var firstCellInRoute = that.getCell(
                        link.route.cellLocations[0].col,
                        link.route.cellLocations[0].row
                    );
                    var lastCellInRoute = that.getCell(
                        link.route.cellLocations[link.route.cellLocations.length - 1].col,
                        link.route.cellLocations[link.route.cellLocations.length - 1].row
                    );
                    routeCellsInThisCol.forEach(function (cell) {
                        ...
                        maxOverlappingVertical = Math.max(
                            maxOverlappingVertical,
                            cell.countVerticalRoutes(excludeStartPoints, excludeEndPoints)
                        ); //todo: should we also keep references to the routes this overlaps?
                    });
        ...
        for (var r = 0; r < this.size; r++) {
            ...
            this.allLinks()
                ...
                .forEach(function (link) {
                    var routeCellsInThisRow = link.route.allCells.filter(function (cell) {
                        return cell.location.row === r;
                    });
                    if (routeCellsInThisRow.length > 0) {
                        // does this route contain this column?
                        var maxOverlappingHorizontal = 0;
                        routeCellsInThisRow.forEach(function (cell) {
                            maxOverlappingHorizontal = Math.max(
                                maxOverlappingHorizontal,
                                cell.countHorizontalRoutes()
                            );
                        });
    ...
    Grid.prototype.calculateOffsets = function (overlaps) {
        ...
        for (var c = 0; c < this.size; c++) {
            ...
            routeOverlaps.forEach(function (routeOverlap) {
                ...
                var firstCellInRoute = that.getCell(
                    route.cellLocations[0].col,
                    route.cellLocations[0].row
                );
                var lastCellInRoute = that.getCell(
                    route.cellLocations[route.cellLocations.length - 1].col,
                    route.cellLocations[route.cellLocations.length - 1].row
                );
                ...
                route.allCells
                    ...
                    .forEach(function (cell) {
                        if (cell.containsHorizontalSegmentOfRoute(route)) {
                            lineNumber = Math.max(
                                lineNumber,
                                numRoutesProcessed[cell.location.col]
                            );
                        }
                    });
    ...
    function isBlockOutsideGrid(block, grid) {
        ...
        return (
            ...
            block.y < 0 ||
            block.y > maxPosition ||
            block.x + (block.blockSize - 1) > maxPosition
        );
    }
    ...
    function addBlockLink(nodeA, nodeB, logicA, logicB, addToLogic) {
        if (nodeA && nodeB) {
            ...
            var linkKey = isFixedNameLink(nodeA, nodeB)
                ? 'blockLink-' + nodeA + '-' + nodeB
                : 'blockLink' + realityEditor.device.utilities.uuidTime();
    ...
    function isFixedNameLink(nodeA, nodeB) {
        return (
            (isInOutBlock(nodeA) || isEdgePlaceholderBlock(nodeA)) &&
            (isInOutBlock(nodeB) || isEdgePlaceholderBlock(nodeB))
        );
    }

    function uploadLinkIfNecessary(blockLink, linkKey) {
        var keys = realityEditor.gui.crafting.eventHelper.getServerObjectLogicKeys(
            globalStates.currentLogic
        );
        realityEditor.network.postNewBlockLink(
            keys.ip,
            keys.objectKey,
            keys.frameKey,
            keys.logicKey,
    ...
    function addBlock(x, y, blockJSON, globalId, isEdgeBlock) {
        ...
        if (realityEditor.gui.crafting.eventHelper.shouldUploadBlock(block)) {
            ...
            realityEditor.network.postNewBlock(
                keys.ip,
                keys.objectKey,
                keys.frameKey,
                keys.logicKey,
    ...
    function removeBlockLink(linkKey) {
        ...
        realityEditor.network.deleteBlockLinkFromObject(
            keys.ip,
            keys.objectKey,
            keys.frameKey,
            keys.logicKey,
    ...
    function removeBlock(logic, blockID) {
        ...
        if (realityEditor.gui.crafting.eventHelper.shouldUploadBlock(logic.blocks[blockID])) {
            ...
            realityEditor.network.deleteBlockFromObject(
                keys.ip,
                keys.objectKey,
                keys.frameKey,
                keys.logicKey,
    ...
    function preprocessPointsForDrawing(points) {
        //... only ever used here.. could just inline it
        ...
        percentages.push.apply(
            percentages,
            lengths.map(function (length) {
                prevPercent += length / totalLength;
                return prevPercent;
            })
        );
