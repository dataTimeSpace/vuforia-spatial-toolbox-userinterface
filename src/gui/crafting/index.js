...
realityEditor.gui.crafting.updateGrid = function (grid) {
    ...
    if (logic) {
        ...
        if (
            ...
            grid.parentLogic() &&
            grid.parentLogic().uuid === globalStates.currentLogic.uuid &&
            blockContainer
        ) {
            // reset domElements
            ...
            for (var blockKey in logic.blocks) {
                ...
                if (
                    realityEditor.gui.crafting.grid.isBlockOutsideGrid(block, grid) &&
                    !block.isPortBlock
                ) {
                    // don't render blocks offscreen
                    continue;
                }
...
realityEditor.gui.crafting.getBlockForDom = function (blockDomElement) {
    ...
    for (var blockKey in globalStates.currentLogic.blocks) {
        ...
        if (
            globalStates.currentLogic.guiState.blockDomElements[block.globalId] === blockDomElement
        ) {
            return block;
        }
    }
...
*/
    ...
    if (!isTempBlock) {
        //TODO: is there a way to set position for new blocks consistently?
        var firstCell = this.grid.getCellForBlock(grid, block, 0);
        var firstCellCenterX = grid.getCellCenterX(firstCell);
        blockDomElement.style.left =
            firstCellCenterX - grid.blockColWidth / 2 + blockOutlinePadding / 2 + 'px';
        blockDomElement.style.top =
            grid.getCellCenterY(firstCell) -
            grid.blockRowHeight / 2 +
            blockOutlinePadding / 2 +
            'px';
    }

    blockDomElement.style.width =
        this.grid.getBlockPixelWidth(block, grid) - blockOutlinePadding + 'px';
    ...
    if (block.x !== -1 && block.y !== -1) {
        realityEditor.gui.moveabilityCorners.wrapDivInOutline(
            blockDomElement,
            8,
            true,
            null,
...
realityEditor.gui.crafting.getBlockIcon = function (logic, blockName, labelSwitch) {
    ...
    if (realityEditor.gui.crafting.blockIconCache[keys.logicKey][blockName] === undefined) {
        ...
        icon.src = realityEditor.network.getURL(
            keys.ip,
            keys.port,
            '/logicBlock/' + blockName + '/icon.svg'
        );
        realityEditor.gui.crafting.blockIconCache[keys.logicKey][blockName] = icon;

        var label = new Image();
        label.src = realityEditor.network.getURL(
            keys.ip,
            keys.port,
            '/logicBlock/' + blockName + '/label.svg'
        );
...
realityEditor.gui.crafting.getSrcForCustomIcon = function (logic) {
    ...
    if (keys) {
        return realityEditor.network.getURL(
            ...
            keys.port,
            '/logicNodeIcon/' +
                realityEditor.getObject(keys.objectKey).name +
                '/' +
                keys.logicKey +
                '.jpg'
        );
...
realityEditor.gui.crafting.getSrcForAutoIcon = function (logic) {
    var validBlockIDs = Object.keys(logic.blocks).filter(function (id) {
        return (
            !realityEditor.gui.crafting.grid.isInOutBlock(id) &&
...
realityEditor.gui.crafting.redrawDataCrafting = function () {
    ...
    if (cutLine.start && cutLine.end) {
        this.realityEditor.gui.ar.lines.drawSimpleLine(
            ctx,
            cutLine.start.x,
            cutLine.start.y,
            cutLine.end.x,
    ...
    let numReusableUpdates =
        connectedInputColors.filter(function (value) {
            return value;
        }).length +
        connectedOutputColors.filter(function (value) {
            return value;
        }).length;
    ...
    connectedInputColors.forEach(function (isConnected, index) {
        ...
        _this.reusableLinkObject.route.pointData.points = [
            { screenX: linkX, screenY: 0 },
            { screenX: linkX, screenY: endY },
        ];
        _this.drawDataCraftingLineDashed(
            ctx,
            realityEditor.gui.crafting.reusableLinkObject,
            numReusableUpdates
        );
    ...
    connectedOutputColors.forEach(function (isConnected, index) {
        ...
        _this.reusableLinkObject.route.pointData.points = [
            { screenX: linkX, screenY: startY },
            { screenX: linkX, screenY: window.innerHeight },
        ];
        _this.drawDataCraftingLineDashed(
            ctx,
            realityEditor.gui.crafting.reusableLinkObject,
    ...
    if (tappedContents) {
        ...
        globalStates.currentLogic.guiState.tempIncomingLinks.forEach(function (linkData) {
            var startCell = _this.grid.getCellForBlock(
                grid,
                _this.grid.blockWithID(linkData.nodeA, globalStates.currentLogic),
                linkData.logicA
            );
            ...
            var xOffset =
                0.5 * grid.blockColWidth +
                (grid.blockColWidth + grid.marginColWidth) * linkData.logicB;
            ...
            _this.realityEditor.gui.ar.lines.drawSimpleLine(
                ctx,
                startX,
                startY,
                endX,
        ...
        globalStates.currentLogic.guiState.tempOutgoingLinks.forEach(function (linkData) {
            var xOffset =
                0.5 * grid.blockColWidth +
                (grid.blockColWidth + grid.marginColWidth) * linkData.logicA;
            var startX = parseInt(domElement.style.left) + xOffset;
            var startY = parseInt(domElement.style.top) + domElement.clientHeight / 2;

            var endCell = _this.grid.getCellForBlock(
                grid,
                _this.grid.blockWithID(linkData.nodeB, globalStates.currentLogic),
                linkData.logicB
            );
 ...
 */
realityEditor.gui.crafting.drawDataCraftingLineDashed = function (
    context,
    linkObject,
    numSharingLinkObject
) {
    if (typeof numSharingLinkObject === 'undefined') {
        numSharingLinkObject = 1;
    }
...
realityEditor.gui.crafting.drawDataCraftingLine = function (
    context,
    linkObject,
    lineStartWeight,
    startColor,
...
realityEditor.gui.crafting.craftingBoardVisible = function (objectKey, frameKey, nodeKey) {
    ...
    this.cout(
        'craftingBoardVisible for object: ' +
            objectKey +
            ', frame: ' +
            frameKey +
            ' and node: ' +
    ...
    if (DEBUG_DATACRAFTING) {
        // TODO: BEN DEBUG - turn off debugging!
...
realityEditor.gui.crafting.blockMenuVisible = function () {
    if (
        document.getElementById('nodeSettingsContainer') &&
        document.getElementById('nodeSettingsContainer').style.display !== 'none'
    ) {
        return;
    }
    ...
    if (existingMenu) {
        ...
        this.blockMenu.initializeBlockMenu(
            function () {
...
realityEditor.gui.crafting.addDatacraftingEventListeners = function () {
    if (globalStates.currentLogic) {
        ...
        realityEditor.device.utilities.addBoundListener(
...
realityEditor.gui.crafting.removeDatacraftingEventListeners = function () {
    if (globalStates.currentLogic) {
        ...
        realityEditor.device.utilities.removeBoundListener(
            datacraftingEventDiv,
            'pointerdown',
            this.eventHandlers.onPointerDown
        );
...
realityEditor.gui.crafting.initializeDataCraftingGrid = function (logic) {
            ...
            for (var colNum = 0; colNum < logic.grid.size; colNum++) {
                if (colNum % 2 === 0) {
                    ...
                    if (rowNum === 0 || rowNum === 6) {
                        blockPlaceholder.style.border =
                            '3px solid ' +
                            realityEditor.gui.crafting.blockColorMap[colNum / 2] +
                            '55'; //rgb(45, 255, 254);"
    ...
    var portCells = logic.grid.cells.filter(function (cell) {
        return (
            cell.canHaveBlock() &&
            (cell.location.row === 0 || cell.location.row === logic.grid.size - 1)
        );
    });
...
realityEditor.gui.crafting.initLogicInOutBlocks = function () {
    for (var y = -1; y <= 4; y += 5) {
        ...
        for (var x = 0; x <= 3; x++) {
            ...
            var activeInputs =
                y === -1 ? [false, false, false, false] : [true, false, false, false];
            var activeOutputs =
                y === -1 ? [true, false, false, false] : [false, false, false, false];
            var blockJSON = this.utilities.toBlockJSON(
                type,
                name,
                1,
                {},
