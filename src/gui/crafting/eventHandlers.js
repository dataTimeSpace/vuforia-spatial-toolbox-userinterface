...
(function (exports) {
    ...
    function onPointerMove(e, setStateMove) {
        ...
        var cell = this.crafting.eventHelper.getCellOverPointer(e.pageX, e.pageY);
        if (!cell) {
            // moved to sidebar menu
            if (touchState !== TS_MOVE) {
                return this.onPointerUp(e);
            }
        } else {
            this.crafting.eventHelper.updateCraftingBackgroundVisibility(
                'move',
                cell,
        ...
        if (touchState === TS_TAP_BLOCK) {
            ...
            if (!this.crafting.eventHelper.areCellsEqual(cell, tappedContents.cell)) {
                ...
                if (Date.now() - startTapTime > globalStates.craftingMoveDelay) {
                    this.cout(
                        'enough time has passed -> HOLD (' + (Date.now() - startTapTime) + ')'
                    );
                    touchState = TS_HOLD;
                    clearTimeout(activeHoldTimer);
                    this.crafting.eventHelper.styleBlockForHolding(
                        globalStates.currentLogic.guiState.tappedContents,
                        true
                    );
                }
            ...
            var didSnap = this.crafting.eventHelper.snapBlockToCellIfPossible(
                tappedContents,
                cell,
                e.pageX,
                e.pageY
            ); //TODO: move to inside the canPlaceBlockInCell block to avoid redundant checks
            if (!didSnap) {
                this.crafting.eventHelper.moveBlockDomToPosition(tappedContents, e.pageX, e.pageY);
                this.crafting.eventHelper.unhighlightPlaceholderDivs(
                    this.crafting.eventHelper.highlightedPlaceholders
                );
            }
            ...
            if (this.crafting.eventHelper.canPlaceBlockInCell(tappedContents, cell)) {
                ...
                var cellsOver = globalStates.currentLogic.grid.getCellsOver(
                    cell,
                    tappedContents.block.blockSize,
                    tappedContents.item
                );
                cellsOver.forEach(function (thisCell) {
                    realityEditor.gui.crafting.eventHelper.stylePlaceholder(
                        { cell: thisCell },
                        true
                    );
                });
    ...
    function onPointerUp(e) {
        ...
        if (touchState === TS_TAP_BLOCK) {
            ...
            if (contents && this.crafting.eventHelper.canConnectBlocks(tappedContents, contents)) {
                this.crafting.eventHelper.createLink(
                    tappedContents,
                    contents,
                    globalStates.currentLogic.guiState.tempLink
                );
            ...
            var isOverSidebar =
                e.pageX > window.innerWidth - (realityEditor.gui.crafting.menuBarWidth + 20);
            if (isOverSidebar) {
    ...
    function onLoadBlock(object, frame, logic, block, publicData) {
        ...
        document
            .getElementById('blockSettingsContainer')
