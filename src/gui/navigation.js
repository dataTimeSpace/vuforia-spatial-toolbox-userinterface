...
(function (exports) {
    ...
    setInterval(() => {
        ...
        newObjectIDs
            .filter((id) => !trackedObjectIDs.includes(id))
            .forEach((id) => {
    ...
    const pathToMesh = (path) => {
        ...
        for (let i = path.length - 1; i > 0; i--) {
            ...
            const startRampHeight =
                lightDistanceTraveled >= Math.abs(rampLength)
                    ? 0
                    : (rampLength - lightDistanceTraveled) * rampRatio;
            const endRampHeight =
                lightDistanceTraveled + direction.length() >= Math.abs(rampLength)
                    ? 0
                    : (rampLength - (lightDistanceTraveled + direction.length())) * rampRatio;

            const startTaperFactor =
                lightDistanceTraveled >= Math.abs(rampLength)
                    ? 1
                    : lightDistanceTraveled / rampLength;
            const endTaperFactor =
                lightDistanceTraveled + direction.length() >= Math.abs(rampLength)
                    ? 1
                    : (lightDistanceTraveled + direction.length()) / rampLength;

            // First top triangle
            topVertices.push(
                start.x - cross.x * startTaperFactor,
                start.y + pathHeight * startTaperFactor + startRampHeight,
                start.z - cross.z * startTaperFactor
            );
            topVertices.push(
                start.x + cross.x * startTaperFactor,
                start.y + pathHeight * startTaperFactor + startRampHeight,
            ...
            topVertices.push(
                end.x - cross.x * endTaperFactor,
                end.y + pathHeight * endTaperFactor + endRampHeight,
                end.z - cross.z * endTaperFactor
            );
            ...
            // First left triangle
            wallVertices.push(
                start.x - bottomCross.x * startTaperFactor,
                start.y + startRampHeight,
                start.z - bottomCross.z * startTaperFactor
            );
            wallVertices.push(
                start.x - cross.x * startTaperFactor,
                start.y + pathHeight * startTaperFactor + startRampHeight,
            ...
            wallVertices.push(
                end.x - bottomCross.x * endTaperFactor,
                end.y + endRampHeight,
                end.z - bottomCross.z * endTaperFactor
            );
            ...
            if (i > 1) {
                ...
                topVertices.push(
                    end.x - nextCross.x * endTaperFactor,
                    end.y + pathHeight * endTaperFactor + endRampHeight,
                    end.z - nextCross.z * endTaperFactor
                );
                ...
                wallVertices.push(
                    end.x - nextBottomCross.x * endTaperFactor,
                    end.y + endRampHeight,
                    end.z - nextBottomCross.z * endTaperFactor
                );
            ...
            while (segLengthRemaining > lightInterval - (lightDistanceTraveled % lightInterval)) {
                const intervalDistanceTraveled =
                    lightInterval - (lightDistanceTraveled % lightInterval);
                ...
                const isLightOn =
                    (lightDistanceTraveled / lightSpeed + Date.now()) % lightTimingInterval <
                    lightOnDuration;
                if (isLightOn) {
        ...
        topGeometry.setAttribute(
            'position',
            new THREE.BufferAttribute(new Float32Array(topVertices), 3)
        );
        wallGeometry.setAttribute(
            'position',
            new THREE.BufferAttribute(new Float32Array(wallVertices), 3)
        );
    ...
    const addNavigationPath = (goalID) => {
        const THREE = realityEditor.gui.threejsScene.THREE;
        const navmeshesWithNode = realityEditor.sceneGraph
            .getObjects()
            .map((sceneNode) => {
                return {
                    sceneNode,
                    navmesh: JSON.parse(
                        window.localStorage.getItem(`realityEditor.navmesh.${sceneNode.id}`)
                    ),
                };
            })
        ...
        if (navmeshesWithNode.length > 0) {
            ...
            const cameraRelativeMatrix = realityEditor.sceneGraph
                .getCameraNode()
                .getMatrixRelativeTo(areaTargetNode);
            const goalRelativeMatrix = realityEditor.sceneGraph
                .getSceneNodeById(goalID)
                .getMatrixRelativeTo(areaTargetNode);
            const cameraRelativeTranslationMatrix =
                realityEditor.gui.ar.utilities.extractTranslation(cameraRelativeMatrix);
            const goalRelativeTranslationMatrix =
                realityEditor.gui.ar.utilities.extractTranslation(goalRelativeMatrix);
            ...
            const relativePath = indexPath
                .map((index) => indexToPos(navmesh, index))
                .map((pos) => scalePos(pos, 1000))
                .map((point) => new THREE.Vector3(point.x, point.y + pathHeightOffset, point.z));
            relativePath[0].x = cameraRelativePosition.x;
            relativePath[0].z = cameraRelativePosition.z;
            relativePath.push(
                new THREE.Vector3(
                    goalRelativePosition.x,
                    goalRelativePosition.y,
                    goalRelativePosition.z
                )
            );

            const pathMesh = pathToMesh(relativePath);

            realityEditor.gui.threejsScene.addToScene(pathMesh, {
                worldObjectId: areaTargetNode.id,
                occluded: true,
            });
    ...
    const findNearestValidIndex = (map, index) => {
        ...
        while (queue.length != 0) {
            ...
            if (
                visitedArray.some(
                    (visitedIndex) =>
                        visitedIndex.x === currentIndex.x && visitedIndex.y === currentIndex.y
                )
            ) {
                continue;
            }
            ...
            queue.splice(
                0,
                0,
                ...neighborPosArray.map((offset) => {
                    return { x: currentIndex.x + offset.x, y: currentIndex.y + offset.y };
                })
            );
        }
    };
    ...
    const findPath = (navmesh, startIndex, goalIndex) => {
        ...
        while (openSet.length > 0) {
            ...
            if (current.x === goalX && current.y === goalY) {
                return simplifyPath(
                    grid,
                    reconstructPath(pathGrid, pathGrid[current.x][current.y])
                );
            }
            const neighborPosArray = [
                { x: -1, y: -1 },
            ...
            neighborPosArray.forEach((neighborPos) => {
                ...
                if (
                    newPos.x < 0 ||
                    newPos.x >= pathGrid.length ||
                    newPos.y < 0 ||
                    newPos.y >= pathGrid[0].length
                ) {
                    return;
                }
                if (pathGrid[newPos.x][newPos.y].value === 0) {
                    // Wall
                    return;
                }
                ...
                const tentativeGScore =
                    current.gScore +
                    Math.sqrt(neighborPos.x * neighborPos.x + neighborPos.y * neighborPos.y);
                if (neighbor.gScore === undefined || tentativeGScore < neighbor.gScore) {
                    neighbor.cameFrom = { x: current.x, y: current.y };
                    neighbor.gScore = tentativeGScore;
                    neighbor.fScore =
                        tentativeGScore + findPathHeuristic(neighbor.x, neighbor.y, goalX, goalY);
                    if (!openSet.some((elem) => elem.x === newPos.x && elem.y === newPos.y)) {
                        const insertIndex = openSet.findIndex(
                            (elem) => elem.fScore < neighbor.fScore
                        ); // Keep smallest on top
    ...
    const simplifyPath = (grid, path) => {
        const pathCopy = path.map((point) => {
            return { x: point.x, y: point.y };
        });
        let currentIndex = 0;
        while (currentIndex < pathCopy.length - 2) {
            while (
                currentIndex + 2 < pathCopy.length &&
                lineIsUnobstructed(grid, pathCopy[currentIndex], pathCopy[currentIndex + 2])
            ) {
