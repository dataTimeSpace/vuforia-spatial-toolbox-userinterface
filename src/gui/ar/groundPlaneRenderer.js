...
(function (exports) {
    ...
    function startVisualization() {
        ...
        if (!gridHelper) {
            const colorGrid = new THREE.Color(
                realityEditor.device.environment.variables.groundWireframeColor
            );
            // scene scale is in milimeters
            gridHelper = new InfiniteGridHelper(
                gridSquareSizeInMm,
                gridRegionSizeInMm,
                0.075,
                colorGrid,
                maxVisibilityDistanceInMm
            );
        ...
        if (!target && realityEditor.device.environment.variables.waitForARTracking) {
            ...
            Object.values(corners).forEach((info) => {
                ...
                let horizontal = new THREE.Mesh(
                    new THREE.BoxGeometry(cornerHeight, cornerHeight, cornerSize),
                    new THREE.MeshBasicMaterial({ color: cornerColor })
                );
                horizontal.position.set(-cornerSize / 2, 0, 0);
                corner.add(horizontal);

                let vertical = new THREE.Mesh(
                    new THREE.BoxGeometry(cornerSize, cornerHeight, cornerHeight),
                    new THREE.MeshBasicMaterial({ color: cornerColor })
                );
    ...
    function onUpdate(_visibleObjects) {
        ...
        if (!shouldVisualize) {
            return;
        } // TODO: actively unsubscribe on stop, so we don't have to ignore loop here
        ...
        if (target) {
            ...
            let raycastIntersects = realityEditor.gui.threejsScene.getRaycastIntersects(
                centerPoint.x,
                centerPoint.y,
                [cachedGroundPlaneCollider.getInternalObject()]
            );
            ...
            target.position.set(
                raycastIntersects[0].scenePoint.x,
                0,
                raycastIntersects[0].scenePoint.z
            );
        }
    }
