...
(function (exports) {
    ...
    var knownObjects = {};
    try {
        knownObjects = JSON.parse(
            window.localStorage.getItem('realityEditor.memory.knownObject') || '{}'
        );
    } catch (e) {
    ...
    MemoryContainer.prototype.set = function (obj) {
        this.obj = obj;
        var urlBase = realityEditor.network.getURL(
            obj.ip,
            realityEditor.network.getPort(obj),
            '/obj/' + obj.name + '/memory/'
        );
        ...
        var cameraMatrix =
            obj.memoryCameraMatrix || realityEditor.gui.ar.utilities.newIdentityMatrix();
    ...
    MemoryContainer.prototype.onTouchStart = function (event) {
        if (!realityEditor.gui.pocket.pocketShown()) {
            // we use the same memory container for pointers and pocket buttons - prevent certain events if in pointer
            return;
        }
    ...
    MemoryContainer.prototype.stopDragging = function () {
        ...
        if (isBar) {
            ...
            if (
                imageRect.left - this.dragDelta.left >
                rightMostContainer.element.getBoundingClientRect().right
            ) {
                this.clear();
                return;
            }
    ...
    function createMemory() {
        ...
        if (sceneNode) {
            ...
            currentMemory.matrix = realityEditor.gui.ar.utilities.copyMatrix(
                realityEditor.gui.ar.draw.visibleObjects[currentMemory.id]
            );
        }
        let cameraNode = realityEditor.sceneGraph.getSceneNodeById('CAMERA');
        currentMemory.cameraMatrix = realityEditor.gui.ar.utilities.copyMatrix(
    ...
    function addKnownObject(objectId) {
        ...
        window.localStorage.setItem(
            'realityEditor.memory.knownObject',
            JSON.stringify(knownObjects)
        );
    }
    ...
    function memoryCanCreate() {
        ...
        visibleObjectKeys.splice(
            visibleObjectKeys.indexOf(realityEditor.worldObjects.getLocalWorldId()),
            1
        ); // remove the local world object, its server cant support memories
    ...
    function uploadImageToServer() {
        ...
        var postUrl = realityEditor.network.getURL(
            objects[currentMemory.id].ip,
            realityEditor.network.getPort(objects[currentMemory.id]),
            '/object/' + currentMemory.id + '/memory'
        );
