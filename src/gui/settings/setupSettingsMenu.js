...
(function (exports) {
    function initService() {
        ...
        realityEditor.gui.settings.addToggleWithText(
            'Zone',
            'limit object discovery to zone',
            'zoneState',
            '../../../svg/zone.svg',
            false,
            'enter zone name',
        ...
        realityEditor.gui.settings.addToggle(
            'Power-Save Mode',
            'turns off some effects for faster performance',
            'powerSaveMode',
            '../../../svg/powerSave.svg',
            false,
            function (newValue) {
        ...
        realityEditor.gui.settings.addToggle(
            'Grouping',
            'double-tap background to draw group around frames',
            'groupingEnabled',
            '../../../svg/grouping.svg',
            false,
            function (newValue) {
        ...
        realityEditor.gui.settings.addToggle(
            ...
            'constantly synchronizes with other users',
            'realtimeEnabled',
            '../../../svg/realtime.svg',
            true,
            function (newValue) {
        ...
        realityEditor.gui.settings.addToggle(
            'Show Tutorial',
            'add tutorial frame on app start',
            'tutorialState',
            '../../../svg/tutorial.svg',
            false,
            function (_newValue) {
        ...
        let introToggle = realityEditor.gui.settings.addToggle(
            'Show Intro Page',
            'shows tips on app start',
            'introTipsState',
            '../../../svg/tutorial.svg',
            false,
            function (newValue) {
        ...
        realityEditor.gui.settings
            .addToggle(
                'AR-UI Repositioning',
                'instantly drag frames instead of interacting',
                'editingMode',
                '../../../svg/move.svg',
                false,
                function (newValue) {
        ...
        realityEditor.gui.settings
                ...
                function (newValue, textValue) {
                    if (newValue && textValue.length > 0) {
                        ...
                        if (!isCurrentUrl) {
                            setTimeout(
                                function () {
                                    // load from external server when toggled on with a new url
                                    realityEditor.app.appFunctionCall('loadNewUI', {
                                        reloadURL: textValue,
                                    });
                                }.bind(this),
                                1000
                        ...
                        setTimeout(
                            function () {
                                // reload from local server when toggled off
            ...
            ); // default value is based on the current source

        realityEditor.gui.settings
            .addToggleWithFrozenText(
                'Discovery Server',
                'load objects from static server',
                'discoveryState',
                '../../../svg/discovery.svg',
                false,
                (realityEditor.network.useHTTPS ? 'https' : 'http') + '://...',
        ...
        realityEditor.gui.settings
            .addToggle(
                'Reset Target Cache',
                'clear cache of downloaded target data',
                'resetTargetCache',
                '../../../svg/object.svg',
                false,
                function (newValue) {
        ...
        realityEditor.gui.settings
            .addToggle(
                'Virtualization',
                'enable virtualization and pose detection',
                ...
                { ignoreOnload: true, dontPersist: true }
            )
            .moveToDevelopMenu();

        let toggleCloudUrl = realityEditor.gui.settings.addURLView(
            'Cloud URL',
            'link to access your metaverse',
            'cloudUrl',
            '../../../svg/zone.svg',
            false,
            'unavailable',
        ...
        let toggleNewNetworkId = realityEditor.gui.settings.addToggleWithFrozenText(
            'New Network ID',
            'generate new network id for cloud connection',
            'generateNewNetworkId',
            '../../../svg/object.svg',
            false,
            'unknown',
            function (_newValue) {
        ...
        let toggleNewSecret = realityEditor.gui.settings.addToggleWithFrozenText(
            'New Secret',
            'generate new secret for cloud connection',
            'generateNewSecret',
            '../../../svg/object.svg',
            false,
            'unknown',
        ...
        function processNewSettings(settings) {
            ...
            if (
                ...
                anyChanged = true;
                toggleCloudUrl.onTextCallback(
                    `https://${settings.serverUrl}/stable` +
            ...
            if (anyChanged) {
                document.getElementById('settingsIframe').contentWindow.postMessage(
                    JSON.stringify({
        ...
        if (window.location.host.split(':')[0] === localSettingsHost.split(':')[0]) {
            fetch(
                `${realityEditor.network.useHTTPS ? 'https' : 'http'}://${localSettingsHost}/hardwareInterface/edgeAgent/settings`
            )
                .then((res) => res.json())
                .then((settings) => {
        ...
        let shouldShowIntroModal =
            window.localStorage.getItem('neverAgainShowIntroTips') !== 'true';
