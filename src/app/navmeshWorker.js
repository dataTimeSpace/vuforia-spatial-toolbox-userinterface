...
import {
    ...
    Texture,
    Vector3,
} from '../../thirdPartyCode/three/three.module.js';
...
onmessage = function (evt) {
    ...
    createNavmeshFromFile(fileName)
        .then((navmesh) => {
...
const addTriangle = (array, v1, v2, v3, value, ignoreValue, faceY, steepness) => {
    ...
    const maxZVertex = [v1, v2, v3]
        .filter((vertex) => vertex != minZVertex)
        .reduce(
            (max, current) => (current.z > max.z ? current : max),
            [v1, v2, v3].filter((vertex) => vertex != minZVertex)[0]
        );
    const midZVertex = [v1, v2, v3].filter(
        (vertex) => vertex != minZVertex && vertex != maxZVertex
    )[0];
    if (midZVertex.z === maxZVertex.z) {
        addBottomFlatTriangle(
            array,
            minZVertex,
            midZVertex,
            maxZVertex,
        ...
        splitVertex.x =
            minZVertex.x +
            ((midZVertex.z - minZVertex.z) / (maxZVertex.z - minZVertex.z)) *
                (maxZVertex.x - minZVertex.x);
...
const createNavmesh = (geometry, resolution) => {
    // resolution = number of pixels per meter
    ...
    const loadVertices = (v1, v2, v3, n1, n2, n3) => {
        if (geometry.index) {
            // Have to handle indexed vertices differently from sequential vertices
    ...
    while (
        ...
        if (
            floorDetectionRayDown.intersectTriangle(
                vertexVector1,
                vertexVector2,
                vertexVector3,
                false,
                floorDetectionResultDown
            )
        ) {
        ...
        if (
            floorDetectionRayUp.intersectTriangle(
                vertexVector1,
                vertexVector2,
                vertexVector3,
                false,
                floorDetectionResultUp
            )
        ) {
        ...
        if (
            vertexVector1.y - minY < lowIgnoreHeight &&
            vertexVector2.y - minY < lowIgnoreHeight &&
            vertexVector3.y - minY < lowIgnoreHeight
        ) {
            ignoreWeight = true;
        }
        if (
            vertexVector1.y - minY > highIgnoreHeight &&
            vertexVector2.y - minY > highIgnoreHeight &&
            vertexVector3.y - minY > highIgnoreHeight
        ) {
            ignoreWeight = true;
        }
    ...
    let steepnessMap = makeArray(faceData.length, faceData[0].length, () => {
        return 0;
    });
    ...
    for (let i = 0; i < faceData.length; i++) {
        for (let j = 0; j < faceData[0].length; j++) {
            steepnessMap[i][j] =
                faceData[i][j][1] === 0 ? 0 : faceData[i][j][4] / faceData[i][j][1]; // 0 --- not computed area in the navmesh, un-walkable area
        }
    }
    ...
    faceData.forEach((xRow, x) => {
        xRow.forEach((value, z) => {
            ...
            if (
                x + 1 < faceData.length &&
                z + 1 < faceData[x].length &&
                faceData[x + 1][z + 1][1] === 0
            ) {
                expandedWallMap[x][z] = 1;
                return;
            }
