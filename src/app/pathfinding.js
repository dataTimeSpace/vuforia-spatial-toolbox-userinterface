...
(function (exports) {
    ...
    let MIN_STEEPNESS = 0,
        MAX_STEEPNESS = 25; // slope / tangent of mesh vertex, in degrees
    ...
    function buildQuad(x, y, z, size, index, isWalkable) {
        // (x, y, z) --- top left corner of the quad, size --- side length of the quad
        posArr.push(x, y, z);
    ...
    function worldPosToNavmeshIndex(pos) {
        // converts threejsScene.js threeJsContainerObj coords to navmesh index (i, j)
    ...
    function setMapStartOrEndIndices(i, j, index) {
        ...
        if (steepnessMapData[j][i] < MIN_STEEPNESS || steepnessMapData[j][i] > MAX_STEEPNESS) {
            // todo Steve: changed for 9/18 demo, only consider steepness map
        ...
        indexCount++;
        if (indexCount <= 2) {
            // the first 2 points are strictly start & end points
            if (indexCount === 1) {
                // set start point
                start.i = i;
                start.j = j;
            ...
            // press button to switch from setting start / end points
    ...
    function computeDistance(a, b) {
        // if we can take on diagonals, use Chebyshev distance; otherwise, use Manhattan distance instead
    ...
    function isDiagonal(parent, current) {
        // check if current node is the diagonal node of parent
        return (
            (current.i === parent.i - 1 && current.j === parent.j - 1) ||
            (current.i === parent.i + 1 && current.j === parent.j - 1) ||
            (current.i === parent.i - 1 && current.j === parent.j + 1) ||
            (current.i === parent.i + 1 && current.j === parent.j + 1)
        );
    ...
    function computeGCost(parent, current) {
        // d (current - start)
    ...
    function computeHCost(n) {
        // d (end - current)
        n.hCost = computeDistance(n, end);
        return n.hCost;
    }

    function computeFCost(parent, current) {
        // G cost + H cost, compute current node's g & f cost based on parent node's g cost
    ...
    function findNeighbor(n) {
        ...
        neighbors = neighbors.filter(
            (neighbor) =>
                neighbor.i >= 0 &&
                neighbor.i < mapData[0].length &&
                neighbor.j >= 0 &&
                neighbor.j < mapData.length
        );
    ...
    function findPath() {
        // start & end are objects, containing following fields: start.i, start.j, start.index
        ...
        while (open.length !== 0) {
            ...
            for (let i = 0; i < neighbors.length; i++) {
                ...
                if (
                    steepnessMapData[n.j][n.i] < MIN_STEEPNESS ||
                    steepnessMapData[n.j][n.i] > MAX_STEEPNESS ||
                    isNodeInArray(n, closed)
                )
                    continue; // todo Steve: changed for 9/18 demo, only consider steepness map

                if (!isNodeInArray(n, open)) {
                    // if neighbor not in open[], push it to open[]
                    n.parent = current;
                    open.push(n);
                } else {
                    // if neighbor is already in open[], but has a lower f cost than what's originally in open[], update f cost
                    n.parent = current;
