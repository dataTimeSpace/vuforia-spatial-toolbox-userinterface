...
(function (exports) {
    ...
    function initService() {
        realityEditor.network.addPostMessageHandler('getOAuthToken', (msgData) => {
            ...
            const edgeServer = isCloud(window.location)
                ? window.location.origin + getToolboxEdgeBasePath()
                : (realityEditor.network.useHTTPS ? 'https' : 'http') +
                  `://${object.ip}:${object.port}`;

            let frameName = realityEditor.getFrame(object.objectId, frame).src;
            loadToken(frameName, authorizationUrl, clientId, edgeServer)
                .then((token) => {
