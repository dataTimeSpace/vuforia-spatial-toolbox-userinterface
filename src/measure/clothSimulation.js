...
(function (exports) {
    ...
    function initService() {
        if (
            cachedOcclusionObject === null ||
            cachedOcclusionObject === undefined ||
            inverseGroundPlaneMatrix === null ||
            inverseGroundPlaneMatrix === undefined
        ) {
            intervalId = setInterval(() => {
                if (
                    ...
                    if (realityEditor.device.environment.isDesktop()) {
                        isOnDesktop = true;
                        raycastPosOffset = new THREE.Vector3().setFromMatrixPosition(
                ...
                if (
                    ...
                    let cachedOcclusionObjectTemp =
                        realityEditor.gui.threejsScene.getObjectForWorldRaycasts(worldId);
                    if (cachedOcclusionObjectTemp === undefined) return;
                    cachedOcclusionObject = [
                        cachedOcclusionObjectTemp.children[0],
                        cachedOcclusionObjectTemp.children[1],
                    ];
                }
    ...
    function setupEventListeners() {
        realityEditor.network.addPostMessageHandler(
            ...
            (msgData, fullMessageData) => {
                ...
                if (
                    msgData.uuid === undefined ||
                    msgData.boundingBoxMin === undefined ||
                    msgData.boundingBoxMax === undefined
                )
                    return;
                ...
                initCloth(
                    ...
                    null,
                    msgData.uuid,
                    fullMessageData.object,
                    fullMessageData.frame
                );
    ...
    class Particle {
        ...
        integrate(timesq) {
            ...
            if (length > COLLIDE_THRESHOLD) {
                // todo Steve: besides inside satisfyConstraints we should make sure each particle doesn't move past collision distance & their own constraint,
                ...
                newPos = this.position
                    .clone()
                    .add(this.tmp3.normalize().multiplyScalar(COLLIDE_THRESHOLD));
            }
        ...
        collide(pos) {
            // particle collide with mesh, cannot move further. Should later change this to include bounding off force
    ...
    function makeParticles(x, y, z, meshCenter, dist) {
        ...
        const makeParticlePosition = (xIndex, yIndex, zIndex) => {
            pos.set(
                xIndex * restDistance - xLength / 2 + center.x,
                yIndex * restDistance - yLength / 2 + center.y,
                zIndex * restDistance - zLength / 2 + center.z
            );
    ...
    function makeConstraints(isVisualize = false) {
        ...
        for (let z = 1; z < zSegs; z++) {
            for (let x = 1; x < xSegs; x++) {
                ...
                if (isVisualize) {
                    ...
                    realityEditor.gui.threejsScene.addToScene(
                        new THREE.Line(lineGeo, lineMatYellow),
                        { layers: 1 }
                    );
                    lineGeo = new THREE.BufferGeometry().setFromPoints([
                        particle1.original,
        ...
        for (let z = 1; z < zSegs; z++) {
            ...
            if (isVisualize) {
                ...
                realityEditor.gui.threejsScene.addToScene(new THREE.Line(lineGeo, lineMatYellow), {
                    layers: 1,
                });
            }
        }
    ...
    function makeWind() {
        ...
        winds.push(
            new Wind(
                new THREE.Vector3(0, -(yLength / 2 + WIND_DISTANCE_OFFSET), 0).add(center),
                new THREE.Vector3(0, 1, 0).multiplyScalar(WIND_STRENGTH)
            )
        ); // bottom
        winds.push(
            new Wind(
                new THREE.Vector3(0, 0, zLength / 2 + WIND_DISTANCE_OFFSET).add(center),
                new THREE.Vector3(0, 0, -1).multiplyScalar(WIND_STRENGTH)
            )
        ); // front
        winds.push(
            new Wind(
                new THREE.Vector3(xLength / 2 + WIND_DISTANCE_OFFSET, 0, 0).add(center),
                new THREE.Vector3(-1, 0, 0).multiplyScalar(WIND_STRENGTH)
            )
        ); // right
        winds.push(
            new Wind(
    ...
    function simulateCloth(particles, constraints, pins, winds, initVolume, volume) {
        ...
        particles.map.forEach((particle) => {
            ...
            if (result.length === 0 || result[0].distance > COLLIDE_THRESHOLD) {
                // not collided
            ...
            particle.collide(
                result[0].point.add(diff.normalize().multiplyScalar(COLLIDE_THRESHOLD))
            );
        });
    ...
    function update() {
        ...
        for (const key of Object.keys(CLOTH_INFO)) {
            ...
            let new_volume = getVolume(
                key,
                value.clothMesh.geometry,
                value.tmpTextLabelObj,
                value.tmpTextLabelPos
            );
            if (value.volume === 0) {
                // when first started, the volume is set to 0
            ...
            if (
                new_volume < 0 ||
                (Date.now() - value.startTime > 5000 &&
                    Math.abs((value.volume - new_volume) / value.volume) < 0.00001) ||
                Date.now() - value.startTime > 30000
            ) {
                // if: (1) new volume < 0; (2) after running 5 seconds && change of volume < 0.001%; (3) after running 30 seconds, then count as finished
